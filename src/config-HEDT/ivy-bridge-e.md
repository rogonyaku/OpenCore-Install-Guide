# HEDT Sandy и Ivy Bridge-E

| Поддержка | Версия |
| :--- | :--- |
| Поддерживаемая версия OpenCore | 0.6.7 |
| Начало поддержки в macOS | OS X 10.9, Mavericks |
| Примечание | Sandy Bridge-E также применим |
...
Эта запись относится к контроллеру Intel I350, который встречается на многих серверных платах Intel и Supermicro, а также на некоторых серверных адаптерах Intel PCIe. Здесь мы будем обманом заставлять драйвер Apple I210 поддерживать наш сетевой контроллер I350:

| Key | Type | Value |
| :--- | :--- | :--- |
| device-id | Data | 33150000 |

* **Примечание**: Если на вашей плате нет встроенной сетевой карты I350 — нет причин добавлять эту запись.
* **Примечание 2**: Если вы получаете панику ядра на кекст AppleIntelI210Ethernet или же отображаются не все адаптеры показываются, вам может понадобиться отредактировать или добавить дополнительные свойства PciRoot для каждого адаптера.
...
Эти значения основаны на рассчитанных значениях в [Отладке OpenCore](../troubleshooting/debug.md)
...
На примере этого Ivy Bridge-E, мы выберем MacPro6,1 SMBIOS.
