# HEDT Skylake-X/W и Cascade Lake-X/W
...
![ACPI](../../img/config/config-universal/x299-acpi.png)
...
| **[SSDT-RTC0-RANGE](https://dortania.github.io/Getting-Started-With-ACPI/)** | Требуется для включения legacy RTC часов в macOS, это также для всех пользователей Big Sur, чтобы убедиться, что их RTC устройство совместимо, см. [Руководство по началу работы с ACPI](https://dortania.github.io/Getting-Started-With-ACPI/) для получения дополнительной информации. |
...
| SetupVirtualMap | YES | Обратите внимание, что новые версии Asus BIOS(v3006+) не будут загружаться с этим квирком |
...
* **SetupVirtualMap**: YES
  ...
  * Обратите внимание, что новые версии Asus BIOS(v3006+) не будут загружаться с этим квирком
* **SyncRuntimePermissions**: YES
...
Требуется для спуфинга (подмены) неподдерживаемых процессоров, к счастью, и Skylake-X, и Cascade Lake-X имеют такой же CPU ID как у чипов Xeon W, которые поставляются в iMac Pro. Поэтому, мы оставим это поле пустым
...
На примере этого Skylake-X, мы выберем iMacPro1,1 SMBIOS.
