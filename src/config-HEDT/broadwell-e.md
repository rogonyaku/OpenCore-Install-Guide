...
| Начало поддержки в macOS | OS X 10.11, El Capitan |
...
| **[SSDT-RTC0-RANGE](https://dortania.github.io/Getting-Started-With-ACPI/)** | Требуется для всех пользователей Big Sur, чтобы убедиться, что их RTC устройство совместимо, см. [Руководство по началу работы с ACPI](https://dortania.github.io/Getting-Started-With-ACPI/) для получения дополнительной информации. |
...
![Kernel](../../img/config/config-HEDT/broadwell-e/kernel.png)
...
Требуется для спуфинга (подмены) неподдерживаемых процессоров и включения управления питанием на Haswell-E и Broadwell-E
...
* **DummyPowerManagement**: No
  * Отключает AppleIntelCPUPowerManagement, требуется только для процессоров AMD
...
| :--- | :--- | :--- |
| AppleCpuPmCfgLock | NO | Требуется, если вы используете 10.10 или старее, и не можете отключить `CFG-Lock` в BIOS |
...
![NVRAM](../../img/config/config-HEDT/broadwell-e/nvram.png)
...
| **npci=0x2000** | Это отключает отладку PCI связанную с `kIOPCIConfiguratorPFM64`, альтернативой является `npci=0x3000`, который дополнительно отключает отладку, связанную с `gIOPCITunnelledKey`. Требуется, если загрузка застревает на `PCI Start Configuration`, поскольку есть IRQ конфликты связанные с вашими PCI линиями. [Источник](https://opensource.apple.com/source/IOPCIFamily/IOPCIFamily-370.0.2/IOPCIBridge.cpp.auto.html) |
...
Принудительно перезаписывает переменные NVRAM, обратите внимание, что `Add` **не перезаписывает** значения присутствующие в NVRAM, поэтому значения вроде `boot-args` следует оставить в покое. Из-за проблем с NVRAM на X99, мы изменим следующее:
...
* **LegacyEnable**: YES
  * Позволяет хранить NVRAM в nvram.plist, что необходимо для систем без нативного NVRAM, таких как X99

* **LegacyOverwrite**: YES
* Разрешает перезапись переменных прошивки из nvram.plist, требуется только для систем без нативного NVRAM, таких как X99

* **LegacySchema**
  * Используется для назначения NVRAM переменных, используется с LegacyEnable в YES

* **WriteFlash**: NO
  * Позволяет записывать в флеш-память все добавленные переменные, не совместимо с эмулируемым NVRAM
...
На примере этого Broadwell-E, мы выберем iMacPro1,1 SMBIOS.
...
![UEFI](../../img/config/config-universal/aptio-iv-uefi.png)
