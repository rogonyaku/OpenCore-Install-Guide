...
Этот раздел содержит вкратце то, почему сообщество переходит на OpenCore, и направлен на развеивание нескольких распространенных мифов в сообществе. Кто просто хочет компьютер с macOS — могут пропустить эту страницу.
...
* Лучшая общая безопасность во многих формах:
  * Не нужно отключать System Integrity Protection (защита целостности системы; кратко - SIP)
  ...
  * Настоящая поддержка Secure Boot
    * Как UEFI, так и вариант от Apple
...
* Многие драйверы прошивки слияются в OpenCore:
...
<!-- * i.e. How would you handle SMBIOS and ACPI data injection once you booted boot.efi, but then get kicked out? The changes are already in memory and so trying to undo them can be quite dangerous. This is why Clover's method is frowned upon. нужно перевести -->
...
OpenCore работает так же, как и любой другой загрузчик, поэтому он также уважает другие ОС. Для других ОС, загрузчики которых имеют неправильный путь или имя, вы можете добавить его в раздел BlessOverride.
...
* Мы загружаем macOS [напрямую с серверов Apple](https://github.com/acidanthera/OpenCorePkg/blob/0.6.8/Utilities/macrecovery/macrecovery.py#L125)
...
Хотя, лицензионное соглашение гласит, что macOS должна быть установленна на подлинном компьютере Mac или на виртуальной машине, которая запущена на подлинном компьютере Mac ([секции 2B-i и 2B-iii](https://www.apple.com/legal/sla/docs/macOSBigSur.pdf)) - нет применимого закона, который запрещает это. Однако, сайты которые перепаковывают и модифицируют установщики macOS могут потенциально столкнуться с проблемой [DMCA](https://en.wikipedia.org/wiki/Digital_Millennium_Copyright_Act) и т.п.
...
Из-за проблем, связанных с поддержкой Nvidia в новых версиях macOS, многие пользователи пришли как-то к выводу, что macOS никогда не поддерживала графические процессоры Nvidia и не поддерживает их на данный момент. Тем не менее, Apple по-прежнему поддерживает компьютеры Mac с графическими процессорами Nvidia в своей последней ОС, к примеру модели MacBook Pro 2013 года с графическими процессорами на ядре Kepler.
