...
В macOS присутствует множество аппаратных ограничений, о которых вы должны знать, прежде чем приступать к установке. Это связанно с ограниченным количеством оборудования поддерживаемым Apple, так что мы либо ограничены самой компанией Apple, либо патчами которые были созданы сообществом.
...
* [Какое оборудование не стоит покупать (на английском)](https://dortania.github.io/Anti-Hackintosh-Buyers-Guide/)
  * Общее руководство о том, каких комплектующих стоит избегать и с какими подводными камнями ваше оборудование может столкнуться.
...
* Поддерживаются как 32-битные, так и 64-битные процессоры
  * Однако требуется поддержка вашей архитектуры в ОС; см. раздел ниже «Требования к процессору»
...
* Поддержка Adobe программ
  * Большая часть программ Adobe используют набор инструкций Intel Memfast, что в итоге вызывает фатальные ошибки на процессорах AMD
...
* 32-битная поддержка
  ...
  * Обходное решение — установить [кастомное ядро (на английском)](https://amd-osx.com/download/kernel.html), но при этом вы потеряете поддержку iMessage
...
| [Arrandale(HD Graphics)](https://en.wikipedia.org/wiki/List_of_Intel_graphics_processing_units#Gen5) | 10.6.4 | 10.13.6 | Поддерживается только LVDS, eDP и внешние выходы не поддерживаются |
...
* **Samsung PM981, PM991 и Micron 2200S NVMe SSD**
  * Эти SSD несовместимы из коробки (вызывая паники ядра) и, следовательно, требуют [NVMeFix.kext](https://github.com/acidanthera/NVMeFix/releases), чтобы починить эти паники ядра. Обратите внимание, что эти накопители могут вызывать проблемы с загрузкой, даже с NVMeFix.kext.
...
Лучшим вариантом является получение поддерживаемой карты от Broadcom; см. [руководство по покупке беспроводных устройств (на английском)](https://dortania.github.io/Wireless-Buyers-Guide/) для получения рекомендаций.
...
* **Комбинированный разъем для наушников**
  * Некоторые ноутбуки с комбинированным разъемом для наушников могут быть с не совсем рабочим аудио входом, поэтому придётся использовать либо встроенный микрофон, либо внешнее устройство аудио входа через USB
