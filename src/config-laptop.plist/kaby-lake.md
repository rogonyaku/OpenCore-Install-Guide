...
![ACPI](../../img/config/config-laptop.plist/kaby-lake/acpi.png)
...
![DeviceProperties](../../img/config/config-laptop.plist/kaby-lake/DeviceProperties.png)
...
| ------------------- | ---- | ------- |
| **00001B59** | Laptop | Рекомендуется для HD615, HD620, HD630, HD640 и HD650 |
| **00001659** | Laptop | Альтернативное значение для 00001B59, если у вас есть проблемы с ускорением, рекомендуется для всех HD и UHD620 NUC |
| **0000C087** | Laptop | Recommended for Amber lake's UHD 617 and Kaby lake R's UHD620 Рекомендуется для Amber lake UHD 617 и Kaby lake R UHD620 |
| **00001E59** | NUC | Рекомендуется для HD615 |
| **00001B59** | NUC | Рекомендуется для HD630 |
| **02002659** | NUC | Рекомендуется для HD640/650 |

#### Примечания к конфигурации

* Для всех пользователей `UHD620` (Kaby Lake-R) вам понадобится подмена device-id:

| Key | Type | Value |
| :--- | :--- | :--- |
| device-id | Data | 16590000 |

* Для всех HD6\*\* (пользователей `UHD` это не касается), есть некоторые небольшие проблемы с выводом, когда подключение чего-либо может вызвать блокировку (паника ядра); вот несколько патчей, чтобы смягчить это (Благодарность Rehabman):
  * С 0306 по 0105 (возможно, однажды он объяснит, что он делает)
...
* С 0204 по 0105 (возможно, однажды он объяснит, что он делает)
...
![PlatformInfo](../../img/config/config-laptop.plist/kaby-lake/smbios.png)

::: tip Информация

Для настройки SMBIOS, мы будем использовать приложение [GenSMBIOS](https://github.com/corpnewt/GenSMBIOS) от CorpNewt.

На примере этого Kaby Lake, мы выберем MacBookPro14,1 SMBIOS - это сделано намеренно для совместимости. Типичная разбивка выглядит следующим образом:

| SMBIOS | Тип процессора | Тип GPU | Размер дисплея | Touch ID |
| :--- | :--- | :--- | :--- | :--- |
| MacBookPro14,1 | Двухъядерный 15w(Low End) | iGPU: Iris Plus 640 | 13" | Нет |
| MacBookPro14,2 | Двухъядерный 15w(High End) | iGPU: Iris Plus 650 | 13" | Есть |
| MacBookPro14,3 | Четырёхъядерный 45w | iGPU: HD 630 + dGPU: RP555/560 | 15" | Есть |
| iMac18,1 | Системы NUC | iGPU: Iris Plus 640 | N/A | Нет |
...
![UEFI](../../img/config/config-universal/aptio-v-uefi-laptop.png)
