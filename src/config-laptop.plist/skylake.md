...
![ACPI](../../img/config/config-laptop.plist/skylake/acpi.png)
...
![DeviceProperties](../../img/config/config-laptop.plist/skylake/DeviceProperties.png)
...
| ------------------- | ---- | ------- |
| **00001619** | Laptop | Рекомендуемое значение для HD515, HD520, HD530, HD540, HD550 and P530 |
| **00001E19** | Laptop | Альтернатива для HD515, если у вас есть проблемы с записью выше |
| **00001B19** | Laptop | Рекомендуемое значение для HD510 |
| **00001E19** | NUC | Рекомендуется для HD515 |
| **02001619** | NUC | Рекомендуется для HD520/530 |
| **02002619** | NUC | Рекомендуется для HD540/550 |
| **05003B19** | NUC | Рекомендуется для HD580 |

#### Configuration Notes

* Для HD510 вам нужно использовать подмену `device-id`:
  
| Key | Type | Value |
| :--- | :--- | :--- |
| device-id | Data | 02190000 |

* Для HD550 и P530 (и, возможно, для всех HD iGPU P-серии), вам может потребоваться использовать `device-id` = `16190000`:
...
![PlatformInfo](../../img/config/config-laptop.plist/skylake/smbios.png)

::: tip Информация

Для настройки SMBIOS, мы будем использовать приложение [GenSMBIOS](https://github.com/corpnewt/GenSMBIOS) от CorpNewt.

На примере этого Skylake, мы выберем MacBookPro13,1 SMBIOS. Типичная разбивка выглядит следующим образом:

| SMBIOS | Тип процессора | Тип GPU | Размер дисплея | Touch ID |
| :--- | :--- | :--- | :--- | :--- |
| MacBook9,1 | Двухъядерный 7w(Low End) | iGPU: HD 515 | 12" | Нет |
| MacBookPro13,1 | Двухъядерный 15w(Low End) | iGPU: Iris 540 | 13" | Нет |
| MacBookPro13,2 | Двухъядерный 15w(High End) | iGPU: Iris 550 | 13" | Есть |
| MacBookPro13,3 | Четырёхъядерный 45w | iGPU: HD 530 + dGPU: RP450/455 | 15" | Есть |
| iMac17,1 | NUC Systems | iGPU: HD 530 + R9 290 | N/A | Нет |
