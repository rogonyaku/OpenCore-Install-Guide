...
* [Помехи на экране ноутбука](#помехи-на-экране-ноутбука)
...
Этот раздел разделен на 4 части, поэтому обратите особое внимание на:

* [Проблемы с Booter](#проблемы-с-booter)
* [Проблемы с патчами ядра](#проблемы-с-патчами-ядра)
* [Проблемы с UEFI](#проблемы-с-uefi)
* [Проблемы на виртуальных машинах](#проблемы-на-виртуальных-машинах)
...
* **DevirtualiseMmio**
  * Некоторые MMIO области по-прежнему требуются для правильного функционирования, поэтому вам нужно либо исключить эти регионы в Booter -> MmioWhitelist, либо отключить этот квирк. Подробнее здесь: [Использование DevirtualiseMmio](../../extras/kaslr-fix.md#использование-devirtualisemmio)
...
* **SetupVirtualMap**
  * Этот квирк требуется для большинства прошивок, и без него, ядро обычно паникует здесь, поэтому включите его, если он отключен
...
* **EnableWriteUnprotector**
  ...
  * Другая проблема может быть в том, что macOS конфликтует с защитой от записи CR0 регистра. Для решения этой проблемы, у нас есть 2 варианта:
    ...
    * Примечание: Некоторые ноутбуки (к примеру, Dell Inspiron 5370) даже с поддержкой MAT не будут загружаться, в таких случаях, вам нужно будет загружаться с комбинацией для старых прошивок(т.е. с EnableWriteUnprotector)
      * Загрузитесь с комбинацией квирков для старых прошивок(т.е. с EnableWriteUnprotector и выключенными `RebuildAppleMemoryMap` с `SyncRuntimePermissions`)
...
Этот раздел разделен между пользователями процессоров Intel и AMD:

#### Процессоры AMD

* Отсутствуют [патчи ядра](https://github.com/AMD-OSX/AMD_Vanilla/tree/opencore) (применимо только для процессоров AMD, и убедитесь, что это патчи для OpenCore, а не для Clover. Clover использует `MatchOS`, когда как у OpenCore `MinKernel` и `MaxKernel`)
...
#### Старые процессоры Intel

У многих прошивок в macOS Big Sur есть проблемы с определением количества ядер ЦП, поэтому ядро паникует слишком рано для отображения ошибки на экране. С помощью последовательного порта, вы можете увидеть следующую панику:
...
* Включите `AvoidRuntimeDefrag` в разделе Booter -> Quirks
  * Это должно сработать на большинстве прошивок

Однако на определенных компьютерах, например HP Compaq DC 7900, прошивка по-прежнему будет приводить к панике, поэтому нам нужно принудительно выставить значение количества ядер процессора. Используйте патч ниже только когда AvoidRuntimeDefrag не работает:
...
Такие же проблемы, что и выше, см. подробнее здесь: [Зависает на `[EB|#LOG:EXITBS:START]`](#зависает-на-eb-log-exitbs-start)
...
::: details Поддерживаемые SMBIOS в macOS Catalina (10.15)
...
::: details Поддерживаемые SMBIOS в macOS Big Sur (11)
...
Что ж, это то место, где PCI устройства устанавливаются и конфигурируются, и где случаются большинство проблем с загрузкой. Другие названия:
...
* **Проблемы с NVMe или SATA**:
  * Иногда, если используется плохой SATA-контроллер или неподдерживаемый NVMe накопитель, вы обычно можете застрять здесь. Что вы можете проверить:
    ...
    * Убедиться, что NVMe-накопитель используется в режиме NVMe в BIOS(некоторые BIOS имеют баг: когда вы можете установить NVMe-накопитель в режим SATA)
* **Ошибки NVRAM**:
...
Пользователям платформы X99 от Intel нужно сделать следующее:

* Следующие патчи ядра должны быть включены:
  * AppleCpuPmCfgLock
  * AppleXcpmCfgLock
  * AppleXcpmExtraMsrs
* У вас должен быть следующий SSDT:
  * SSDT-UNC (если у вас его нет, см. [Начало работы с ACPI](https://dortania.github.io/Getting-Started-With-ACPI/) о том, как его сделать)
...
Пользователям MSI Navi нужно применить патч упомянутый здесь: [Installer not working with 5700XT #901](https://github.com/acidanthera/bugtracker/issues/901)
...
Примечание: В macOS Big Sur больше не требуется этот патч.
...
Пользователи материнских плат Comet Lake с сетевым контроллером i225-V могут получить панику ядра при загрузке с кекстом для i210. Чтобы решить эту проблему, убедитесь, что у вас правильный PciRoot для вашего Ethernet. Обычно это:

* PciRoot(0x0)/Pci(0x1C,0x1)/Pci(0x0, 0x0)
  * По умолчанию этот путь используют материнские платы от Asus и Gigabyte
* PciRoot(0x0)/Pci(0x1C,0x4)/Pci(0x0,0x0)
  * Некоторые OEM-производители могут использовать это, вместо вышеуказанного
...
* Используйте [FakeSMC от Rehabman](https://bitbucket.org/RehabMan/os-x-fakesmc-kozlek/downloads/) (вы можете использовать трюк с `MinKernel`/`MaxKernel` чтобы ограничить FakeSMC в Big Sur и выше)
...
### Проблемы на виртуальных машинах

* Известно, что VMWare 15 застревает на `[EB|#LOG:EXITBS:START]`. На VMWare 16 такой проблемы нет.
