...
![ACPI](../../img/config/config.plist/coffeelake/acpi.png)
...
| **[SSDT-PMC](https://dortania.github.io/Getting-Started-With-ACPI/)** | Настоящие материнские платы 300 серии (не Z370) не объявляет FW чип как MMIO в ACPI, и поэтому XNU игнорирует MMIO регион объявленный UEFI memory map. Этот SSDT возвращает обратно поддержку NVRAM. См. [руководство по Началу работы с ACPI](https://dortania.github.io/Getting-Started-With-ACPI/) для получения подробной информации. |
...
| ProtectUefiServices | YES | Требуется на Z390 системе |
...
* **DevirtualiseMmio**: YES
  * Уменьшает объём Stolen памяти, расширяет возможные значения для `slide=N`, и очень полезен при устранении проблем с выделением памяти (Memory Allocation) на Z390. Требуется также `ProtectUefiServices` на IceLake и Z390 Coffee Lake.
...
![DeviceProperties](../../img/config/config.plist/coffeelake/DeviceProperties.png)
...
| 0300913E | Используется, когда iGPU используется только для вычислительных задач, не управляя дисплеем |
...
Для пользователей с проблемами черного экрана после verbose на B360, B365, H310, H370, Z390, пожалуйста, смотрите страницу [патчинга BusID iGPU](https://dortania.github.io/OpenCore-Post-Install/gpu-patching/intel-patching/)
...
* Вы можете удалить это свойство, поскольку оно не используется нами в настоящее время
...
На примере этого Coffee Lake, мы выберем iMac19,1 SMBIOS - это сделано намеренно для совместимости. Для Coffee Lake используются два основных SMBIOS:

| SMBIOS | Оборудование |
| :--- | :--- |
| iMac19,1 | Для Mojave и новее |
| iMac18,3 | для High Sierra и старее |

* Вы будете использовать 18,3, когда у вас есть Pascal или Maxwell dGPU, и вы ограничены веб-драйверами в версиях macOS
