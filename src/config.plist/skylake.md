...
* [GenSMBIOS](https://github.com/corpnewt/GenSMBIOS)
  * Для генерации наших данных SMBIOS
...
![DeviceProperties](../../img/config/config.plist/skylake/DeviceProperties.png)
...
| :--- | :--- |
| 00001219 | Используется, когда iGPU используется для управления дисплеем |
| 01001219 | Используется, когда iGPU используется только для вычислительных задач, не управляя дисплеем |

Мы также добавим ещё 3 свойства: `framebuffer-patch-enable`, `framebuffer-stolenmem` и `framebuffer-fbmem`. Первый включает патчинг через WhateverGreen.kext, второй устанавливает минимальный размер графической памяти в 19 Мб, и третий устанавливает память фреймбуфера в 9 Мб. Это обычно не нужно, когда это может быть настроено в BIOS (рекомендуется 64 Мб), но требуется, когда настройки недоступны.
...
И дополнительное примечание для пользователей P530, ваш iGPU не поддерживается нативно, поэтому вам нужно будет добавить следующие свойства:

| Key | Type | Value |
| :--- | :--- | :--- |
| device-id | Data | 1B190000 |

Когда вы закончите, у вас должно быть что-то похожее:
...
(Это пример для настольно HD P530 без dGPU и настроек iGPU памяти в BIOS)
