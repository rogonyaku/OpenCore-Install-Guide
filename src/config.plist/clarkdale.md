# Настольные Lynnfield и Clarkdale

| Поддержка | Версия |
| :--- | :--- |
| Поддерживаемая версия OpenCore | 0.6.7 |
| Начало поддержки в macOS | OS X 10.6.3, Snow Leopard |
| Примечание 1 | Apple не поддерживает настольные iGPU Iron Lake|
| Примечание 2 | Многие платы Lynnfield и Clarkdale не поддерживают UEFI |
...
По умолчанию, в Sample.plist этот раздел настроен для iGPU и Audio. Драйверы Apple Iron lake iGPU не поддерживают настольное оборудование, поэтому PciRoot `PciRoot(0x0)/Pci(0x2,0x0)` можно удалить из раздела `Add`. Для звука, мы будем настраивать Layout в boot-args разделе, поэтому удаление `PciRoot(0x0)/Pci(0x1b,0x0)` также рекомендуется из `Add` и `Block` разделов
...
На примере этого Clarkdale, мы выберем iMac11,2 SMBIOS - это сделано намеренно для совместимости. Для Clarkdale используются 3 основных SMBIOS:
