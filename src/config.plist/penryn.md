# Настольные Yonah, Conroe и Penryn
...
| Примечание 2 | SSE4 требуется для загрузки macOS 10.12 Sierra и новее, поэтому Conroe и старее - не поддерживаются |

## Отправная точка

Итак, создание config.plist может показаться сложным, но это не так. Это просто занимает некоторое время, но это руководство расскажет вам как всё сконфигурировать, и вы не останетесь в стороне. Это также означает, что если у вас есть проблемы, проверьте настройки конфигурации, чтобы убедиться, что они правильные. Основные моменты, которые нужно подметить с OpenCore:
...
| Legacy | UEFI |
| :--- | :--- |
| ![](../../img/config/config-legacy/booter-duetpkg.png) | ![](../../img/config/config-universal/aptio-iv-booter-sl.png) |

Этот раздел посвящен квиркам относящимся к патчингу boot.efi с помощью OpenRuntime, заменяющего AptioMemoryFix.efi

### MmioWhitelist

Этот раздел позволяет передавать некие пространства в macOS, которые обычно игнорируются; полезно в сочетании с `DevirtualiseMmio`
...
* **AvoidRuntimeDefrag**: NO
  * Исправляет рантайм-сервисы UEFI, такие как дата, время, NVRAM, управление питанием;
  * Однако macOS Big Sur требует APIC таблицу, в противном случае вызывает раннюю панику ядра, поэтому пользователям Big Sur рекомендуется этот квирк.
...
По умолчанию, в Sample.plist этот раздел настроен для iGPU и Audio. Мы не будем рассматривать iGPU, поэтому PciRoot `PciRoot(0x0)/Pci(0x2,0x0)` можно удалить из раздела `Add`. Для звука, мы будем настраивать Layout в boot-args разделе, поэтому удаление `PciRoot(0x0)/Pci(0x1b,0x0)` также рекомендуется из `Add` и `Block` разделов

TL;DR, удалите все PciRoot здесь, поскольку мы не будем этот раздел.

### Delete

Удаляет Device Properties (свойства устройства) из map, в нашем случае, мы можем игнорировать это.

## Kernel

![](../../img/config/config-legacy/penryn-kernel.png)
...
Напоминаем, что пользователи [ProperTree](https://github.com/corpnewt/ProperTree) могут нажать **Cmd/Ctrl + Shift + R** для добавления всех своих кекстов в правильном порядке без ручного вписывания каждого из них.
...
* **AppleCpuPmCfgLock**: NO
  * CFG-Lock отсутствует на Penryn, так что нет необходимости в этом квирке
* **AppleXcpmCfgLock**: NO
  * CFG-Lock отсутствует на Penryn, так что нет необходимости в этом квирке
* **CustomSMBIOSGuid**: NO
...
* **XhciPortLimit**: NO
  * Фактически, это исправление ограничения на 15 портов, однако он применим только для систем с XHCI контроллерами
...
Эти значения основаны на значениях, рассчитанных в разделе [Отладка OpenCore](../troubleshooting/debug.md)
...
Используется для запуска отладочных инструментов OC, к примеру, Shell; функция снапшота в ProperTree добавит их за вас.
...
| boot-args | Описание |
| :--- | :--- |
| **nvda_drv_vrl=1** | Используется для включения веб-драйверов Nvidia на картах Maxwell и Pascal в Sierra и High Sierra, в Sierra и старее можно использовать `nvda_drv=1` |
...
![PlatformInfo](../../img/config/config.plist/ivy-bridge/smbios.png)

::: tip Информация

Для настройки SMBIOS, мы будем использовать приложение [GenSMBIOS](https://github.com/corpnewt/GenSMBIOS) от CorpNewt.

На примере этого Penryn, мы выберем iMac10,1 SMBIOS - это сделано намеренно для совместимости. Для этого оборудования используются 4 основных SMBIOS:

| SMBIOS | Оборудование | Поддержка ОС |
| :--- | :--- | :--- |
| iMac4,1 | Yonah SMBIOS(32-бит) | 10.4-10.6.8 |
| iMac7,1 | Conroe SMBIOS(64-бит, SSE3) | 10.4-10.11.6 |
| iMac10,1 | Penryn SMBIOS(64-бит, SSE4) | 10.6-10.13.6 |
| MacPro6,1 | SMBIOS для Mojave и новее | 10.9-текущая |

* Если вы планируете запустить macOS 10.14, Mojave или новее, рекомендованным SMBIOS будет MacPro6,1. Однако учтите, что вам понадобится [telemetrap.kext](https://forums.macrumors.com/threads/mp3-1-others-sse-4-2-emulation-to-enable-amd-metal-driver.2206682/page-4?post=28447707#post-28447707), чтобы решить проблемы при установке
...
Мы устанавливаем в Generic -> ROM либо Apple ROM (сдампленный с реального Mac), либо MAC-адрес вашей сетевой карты, или же любой случайный MAC-адрес (всего может быть 6 случайных байтов, для этого руководства мы будем использовать `11223300 0000`. После установки, посетите страницу [Исправление iServices](https://dortania.github.io/OpenCore-Post-Install/universal/iservices.html), чтобы найти, как получить настоящий MAC-адрес)
...
![](../../img/config/config-legacy/uefi-sandy-desktop.png)
...
| Quirk | Enabled | Комментарии |
| :--- | :--- | :--- |
| IgnoreInvalidFlexRatio | NO | Включите это, если у вас UEFI BIOS |
